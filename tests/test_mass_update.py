...
def test_operationmanager_get_for_field() -> None:
...
class MassUpdateTest(SelectRowsMixin, CheckSignalsMixin, WebTestMixin, TestCase):
    ...
    def test_custom_modeladmin_form(self) -> None:
    ...
    def test_messages(self) -> None:
        with user_grant_permission(
            ...
            assert messages
            assert messages[0] == "Updated 2 records"

            res = self._run_action(selected_rows=[1]).follow()
            messages = [m.message for m in list(res.context["messages"])]
            assert messages
            assert messages[0] == "Updated 1 records"

    @skipIf(not celery_present, "Celery not installed")
    def test_async_qs(self) -> None:
        # Create handler
    ...
    def test_file_field(self) -> None:
        self._run_action(
            _validate=1, select_across=1, chk_id_image=True, image=Upload(str(Path(__file__).parent / "test.jpeg"))
        )
