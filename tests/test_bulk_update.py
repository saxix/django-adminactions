...
class BulkUpdate(SelectRowsMixin, CheckSignalsMixin, WebTestMixin):
    ...
    def test_index_required(self) -> None:
    ...
    def test_bulk_update_with_one_to_one_field(self) -> None:
        ...
        assert DemoModel.objects.filter(pk=demo_model_instance.pk, onetoone=demo_one_to_one_instance).exists()
        assert res.status_code == 200

    def test_bulk_update_with_foreign_key(self) -> None:
        demo_model_instance = G(DemoModel, char="InitialValue", integer=123)
        ...
        assert DemoRelated.objects.filter(pk=demo_related_instance.pk, demo=new_demo_model_instance).exists()
        assert res.status_code == 200
